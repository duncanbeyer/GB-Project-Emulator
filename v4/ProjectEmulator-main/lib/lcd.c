...
u8 lcd_read(u16 address) {
    // fp3 = fopen("lcd_dbg.txt", "a");
    u8 offset = (address - 0xFF40);
    u8 *p = (u8 *)&ctx;
    // fprintf(fp3, "reading address %04X value %02X on tick %08lX.\n", address, p[offset], emu_get_context()->ticks);
    // fclose(fp3);
    return p[offset];
}
...
void lcd_write(u16 address, u8 value) {
    ...
    u8 offset = (address - 0xFF40);
    u8 *p = (u8 *)&ctx;
    // fprintf(fp3, "writing address %01X value %02X on tick %08lX.\n", offset, value, emu_get_context()->ticks);
    // fclose(fp3);
    p[offset] = value;
