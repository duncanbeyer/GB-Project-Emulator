...
static void proc_ld(cpu_context *ctx) {
    ...
    if (ctx->current_instruction->mode == A_HL_SPR) {
        ...
        cpu_read_reg(ctx->current_instruction->reg2) + (int8_t)ctx->fetch_data);
...
static void proc_ldh(cpu_context *ctx) {
    ...
    if (ctx->current_instruction->reg1 == R_A) { // If loading into Accumulator
        // fprintf(fp2, "ldh value %02X at %04X to a on tick %08lX\n", bus_read(0xFF00 | ctx->fetch_data), (0xFF00 | ctx->fetch_data), emu_get_context()->ticks);
        ...
        // fprintf(fp2, "ldh value %02X at %04X to %04X on tick %08lX\n", bus_read(0xFF00 | ctx->fetch_data), (0xFF00 | ctx->fetch_data), ctx->memory_destination, emu_get_context()->ticks);
...
static void proc_jp(cpu_context *ctx) {
    ...
    // fprintf(fp2, "jumping to %04X on tick %08lX\n", ctx->fetch_data, emu_get_context()->ticks);
    // fclose(fp2);
...
static void proc_add(cpu_context *ctx) {
    ...
    if (ctx->current_instruction->reg1 == R_SP) {
        ...
        val = cpu_read_reg(ctx->current_instruction->reg1) + (int8_t)ctx->fetch_data;
    }
